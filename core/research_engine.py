...
class ResearchEngine:
    def __init__(self, config: Optional[ResearchConfig] = None):
        self.config = config or ResearchConfig(); self.generator = StrategyGenerator(self.config.max_candidates); self.signal_engine = SignalEngine(); self.backtester = Backtester(BacktestConfig(initial_balance=self.config.initial_balance, risk_percent=self.config.risk_percent)); self.ranker = StrategyRanker(); self.last_results=[]; self.last_ranked=[]; self.last_oos=[]; self.last_walk_forward=[]; self.last_exports={}
    def generate_candidates(self): return self.generator.generate()
    def rank_results(self, results): self.last_ranked=self.ranker.rank(results); return self.last_ranked
    def run_candidate(self, data, signals, sl_distance, rr=None):
        result=self.backtester.run(data,signals,sl_distance,self.config.default_rr if rr is None else rr); trades=result.trades; wins=sum(1 for t in trades if t.pnl>0); gp=sum(t.pnl for t in trades if t.pnl>0); gl=abs(sum(t.pnl for t in trades if t.pnl<0)); pf=gp/gl if gl>0 else (float('inf') if gp>0 else 0.0); win_rate=100*wins/len(trades) if trades else 0.0; net_r=sum(t.r_multiple for t in trades); expectancy=net_r/len(trades) if trades else 0.0; peak=self.config.initial_balance; max_dd_cash=0.0
    ...
    def evaluate_spec(self,data,spec):
        signals,sl_distance=self.signal_engine.build(data,spec); metrics=self.run_candidate(data,signals,sl_distance,rr=float(spec.risk["rr"])); metrics["strategy_name"]=spec.name; metrics["direction"]=spec.direction; metrics["strategy"]=spec.to_dict(); return metrics
