...
The report intentionally excludes bulky raw trade/equity payloads. Those remain local
or in the existing exports; this module publishes the information needed to audit a
research run, reproduce its configuration, understand filtering, and inspect ranking.
"""
...
def _compact_metrics(metrics: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    if not isinstance(metrics, dict):
        return {}
    keep = (
        "trade_count", "win_rate", "profit_factor", "net_r", "expectancy",
        "max_drawdown", "max_drawdown_pct", "max_drawdown_r", "final_balance",
    )
...
def _compact_regime(value: Any) -> Dict[str, Any]:
    ...
    for row in value.get("results", []) or []:
        if isinstance(row, dict):
            regimes.append({
                k: _safe(row.get(k))
                for k in ("regime", "trade_count", "net_r", "profit_factor", "max_drawdown_r", "win_rate")
                if k in row
            })
    return {
        "robustness_score": _metric(value.get("robustness_score")),
        "results": regimes,
...
def _compact_mc(value: Any) -> Dict[str, Any]:
    if not isinstance(value, dict):
        return {}
    keys = (
        "simulations", "probability_positive", "probability_ruin", "p05_net_r",
        "p50_net_r", "p95_net_r", "p05_max_drawdown_r", "p50_max_drawdown_r",
        "p95_max_drawdown_r", "mean_net_r", "std_net_r",
    )
...
def _markdown(report: Dict[str, Any]) -> str:
    ...
    if best:
        ...
        lines += [
            "",
            "### Best Strategy",
            f"- Name: `{bc.get('name', 'unknown')}`",
            f"- Direction: `{bc.get('direction', 'unknown')}`",
            f"- Validation score: `{best.get('validation_score', 0):.4f}`",
            f"- Walk-forward score: `{best.get('walk_forward_score', 0):.4f}`",
            f"- Monte Carlo robustness: `{best.get('monte_carlo_robustness', 0):.2f}`",
            f"- Sensitivity robustness: `{best.get('sensitivity_robustness', 0):.2f}`",
            f"- Regime robustness: `{best.get('regime_robustness', 0):.2f}`",
            f"- OOS Net R: `{best.get('test', {}).get('net_r', 0)}`",
            f"- OOS Profit Factor: `{best.get('test', {}).get('profit_factor', 0)}`",
        ]
    lines += [
        "",
        "## Data",
        ...
        f"- End: `{data.get('end')}`",
        f"- Train bars: `{data.get('train_bars')}`",
        f"- Test bars: `{data.get('test_bars')}`",
        "",
...
def export_research_report(
    ...
    *,
    ...
    report = {
        ...
        "ranking": {
            "final_order": "regime_robustness > sensitivity_robustness > monte_carlo_robustness > walk_forward_score",
            "top_strategies": ranked_items,
        },
    }
    ...
    manifest = {
        ...
        "raw_large_files": ["ranked_strategies.json"],
        "raw_large_files_policy": "local_only_and_gitignored",
        "report_is_compact": True,
    }
