...
class MainWindow(ttk.Frame):
    ...
    def _build(self):
        self.status = StatusPanel(self); self.status.pack(fill="x", padx=8, pady=8)
        ...
        self.progress = ProgressPanel(self); self.progress.pack(fill="x", padx=8, pady=(0, 8))
        self.log = LogPanel(self); self.log.pack(fill="both", expand=True, padx=8, pady=(0, 8))
    ...
    def initial_diagnostics(self):
        ...
        try:
            import MetaTrader5
            self.status.set("Python", platform.python_version()); self.status.set("MetaTrader5 Package", "OK ✓")
            self.write("MetaTrader5 package: OK ✓")
        except ImportError:
            self.status.set("MetaTrader5 Package", "MISSING ✗"); self.write("MetaTrader5 package: MISSING ✗", "ERROR"); return
    ...
    def connect_mt5(self, initial=False):
        def worker():
            ...
            if not d.connected:
                self.status.set("MT5 Terminal", "Not Connected ✗"); self.write(f"MT5: NOT CONNECTED — {d.error}", "ERROR"); return
            self.status.set("MT5 Terminal", "Connected ✓"); self.status.set("Account", d.account or "—"); self.status.set("Server", d.server or "—")
            self.status.set("Symbol", "XAUUSD AVAILABLE ✓" if d.symbol_available else "XAUUSD NOT AVAILABLE ✗")
            self.write("Connected ✓", "MT5"); self.write(f"Terminal: {d.terminal or 'MetaTrader 5'}", "MT5"); self.write(f"Build: {d.build}", "MT5")
            self.write(f"Account: {d.account}", "MT5"); self.write(f"Server: {d.server}", "MT5")
            if d.symbol_available:
                self.write("XAUUSD: AVAILABLE ✓", "MT5"); self.write(f"Digits: {d.digits}", "MT5"); self.write(f"Point: {d.point}", "MT5"); self.write(f"Bid={d.bid} Ask={d.ask}", "MT5")
    ...
    def load_data(self):
        ...
        self.status.set("Timeframe", timeframe); self.status.set("Historical Data", "Loading..."); self.progress.set_phase("DATA"); self.progress.update_progress(0, 1, "Loading historical data...")
        self.write("=======================================", "DATA"); self.write(f"LOAD DATA started: XAUUSD {timeframe}, bars={bars}", "DATA")
        def worker():
            try:
                import pandas as pd
                rates = self.connector.load_rates(timeframe, bars)
                if len(rates) == 0: raise RuntimeError("MT5 returned zero bars.")
                frame = pd.DataFrame(rates); frame["time"] = pd.to_datetime(frame["time"], unit="s"); frame = frame.set_index("time").sort_index()
                last_dt = frame.index[-1].strftime("%Y-%m-%d %H:%M:%S"); self.loaded_data = frame
                self.status.set("Historical Data", "Loaded ✓"); self.status.set("Number of Bars", str(len(frame))); self.status.set("Last Bar Time", last_dt)
                self.after(0, lambda: self.progress.update_progress(1, 1, f"Bars loaded: {len(frame)}")); self.write(f"Historical data loaded ✓ — {len(frame)} bars", "DATA"); self.write(f"Last bar: {last_dt}", "DATA")
    ...
    def start_research(self):
        ...
        self.stop_event.clear(); total = self.engine.config.max_candidates; self.progress.set_phase("RESEARCH"); self.progress.update_progress(0, total)
        self.write("=======================================", "RESEARCH"); self.write("Started", "RESEARCH"); self.write(f"Candidates: {total}", "RESEARCH")
        ...
        def wf_progress(strategy, strategy_total, window, window_total):
            percent = ((strategy - 1) * window_total + window) / max(strategy_total * window_total, 1) * 100.0
            detail = f"Strategy: {strategy} / {strategy_total} | Window: {window} / {window_total}"
            self.after(0, lambda: (self.progress.set_phase("WALK-FORWARD"), self.progress.update_progress(int((percent / 100.0) * 100), 100, detail)))
        ...
        def worker():
            try:
                ...
                if self.stop_event.is_set():
                    self.write("Research stopped by user.", "RESEARCH"); self.after(0, lambda: self.progress.set_phase("STOPPED"))
                else:
                    self.write("Research finished.", "RESEARCH"); self.write(f"Ranked strategies: {len(ranked)}", "RESULT"); self.after(0, lambda: self.progress.set_phase("FINISHED"))
                    if ranked:
                        best = ranked[0]; name = getattr(best.get("candidate"), "name", None) or best.get("strategy_name", "N/A"); score = best.get("walk_forward_score", best.get("validation_score", 0)); pf = best.get("oos_pf_mean", best.get("profit_factor", 0)); netr = best.get("oos_net_r_sum", best.get("net_r", 0))
                ...
                self.write(f"Research failed: {exc}", "ERROR"); self.after(0, lambda: self.progress.set_phase("ERROR"))
